...
exports.addNewUser = async ({ data }) => {
    try {
        const checkExistingEmail = await db.query(
            `select count(1) from users where email = '${data.email}'`
        );
        if (checkExistingEmail.rows[0].count > 0)
    ...
    try {
        ...
        if (runCheckUserQuery.rowCount === 1) {
            if (
                bcrypt.compareSync(
                    data.password,
                    runCheckUserQuery.rows[0].password
                )
            ) {
                const token = jwt.sign(
                    {
                        id: runCheckUserQuery.rows[0].uid,
                        email: runCheckUserQuery.rows[0].email,
                    },
